...
export default function PromoCard() {
  ...
  function showPromo() {
    if (cd.active) return;
    try { navigator.clipboard?.writeText('Divan2025'); } catch {}
    show({
      title: 'Divan2025',
    ...
    <section className={s.container}>
      <div className={s.card}>
        <div>
          ...
          <button className="btn btn-primary" onClick={showPromo} disabled={cd.active}>{cd.active ? `Повторно через ${cd.left}с` : 'Показать промокод'}</button>
        </div>
